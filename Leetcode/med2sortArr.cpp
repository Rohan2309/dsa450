...
double findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2)
{
    ...
    int n = nums1.size();
    int m = nums2.size();
    int low = 0;  //at minimum we can choose 0 elements from 1st arr
    int high = n; //at maximum we can choose n elements from 1st arr
    int len = n + m;
    bool odd = true;
    if (len % 2 == 0)
    {
    ...
    while (low <= high)
    {
        ...
        { //we have to shift cut 1 towards left
            high = cut1 - 1;
        }
        else
        { //we have to shift cut1 towards right
            low = cut1 + 1;
        }
    }
    return 0.0;
