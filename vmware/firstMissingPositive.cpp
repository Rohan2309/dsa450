...
public:
    int firstMissingPositive(vector<int>& nums) {
        int n=nums.size();
        for(int i=0 ; i<n ; i++)
        {// neglecting numbers outside 0 -> n
            //if positive ints ot in correct pos, we swap and get it in correct pos
            while(nums[i]>0 and nums[i]<=n and nums[i]!=nums[nums[i]-1])
                swap(nums[i],nums[nums[i]-1]);
        }
        //the number which is not in correct pos
        for(int i=0 ; i<n ; i++) if(nums[i] != i+1) return i+1;
        //if all are in correct pos
        return n+1;//we return next ele
    }
};
