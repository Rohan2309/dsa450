...
public:
    int lengthOfLongestSubstring(string s) {
        map<char, int>mp;//character with current idx
        int n = s.size();
        int left =0;
        int len =0;
        for (int right =0; right< n; right++) {//looping through all elements
            if (mp.find(s[right])!=mp.end()){//if already element is present in map
