...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        ...
        for(int i=0; i< inorder.size(); i++) hash[inorder[i]] = i;// 9->0, 3->1, 15->2....6->4
    ...
    TreeNode* helper(vector<int>& preorder, int pSt, int pEn, vector<int>& inorder, int iSt, int iEn, map<int,int>&hash){
        ...
        if(pSt>pEn or iSt>iEn)return NULL;
        ...
        int inPos = hash[root->val]; //will return position from hash map (of root)
