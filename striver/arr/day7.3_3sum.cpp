...
public:
    vector<vector<int>> threeSum(vector<int>& nums) {
        ...
        for(int i=0; i<n-2; i++){//loop for a, it will go till 3rd lasr element 2nd last b and 3rd last c
            if(i==0 or (i>0 and (nums[i]!= nums[i-1]))){
                ...
                while(low<high){
                    ...
                    if((nums[low]+nums[high])==sum){//b+c=-a
                        ...
                        arr.emplace_back(nums[low]);
                        arr.emplace_back(nums[high]);
                        ans.emplace_back(arr);
                        while(low< high and (nums[low]==nums[low+1])){//while b is same skipping
                            low++;//coming till last same b
                        }
                        low++;//taking different b(next)
                        while(low<high and(nums[high]==nums[high-1])){//while c is same skipping
