...
Medium Accuracy: 39.46% Submissions: 82690 Points: 4
...
public:
    ...
    {
        // code here
        int in[n] = {0};  //how many people knows that person
        int out[n] = {0}; //how many people does that person knows

        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < n; j++)
            {
                if (M[i][j] == 1) //when 1 that means i knows j
                {
        ...
        for (int i = 0; i < n; i++)
        {
            if (in[i] == n - 1 and out[i] == 0) //known by all person except oneself and doesn't knows anyone
            {
                return i;
            }
        }
