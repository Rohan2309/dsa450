...
{
    // calculating sum of array sum
    int totSum = 0; //rest element sum
    int ans = 0; //counter
    int subset = 0; //subset sum
    for (int i = 0; i < n; i++)
        totSum += arr[i];   
 
    // sort the array r.
    sort(arr, arr + n);
    int j = n-1; //iterator from last
    while(true){
        subset+=arr[j]; //taking subset with largest number
        totSum-=arr[j]; //reducing total sum
        j--; //decre pointer
        ans++; //taking one one element in subset
        if(subset>totSum){ //when subset sum is larger than remaining sum
            break;
        }
    }
    return ans;
